...
const FamilyMigrationMap = () => {
    ...
    useEffect(() => {
        ...
        const fetchAncestors = async () => {
            const response = await fetch("/api/ancestors"); // Adjust as needed
        ...
        const getOpacity = (relationLevel) => Math.max(100 - relationLevel, 10) / 100; // Min opacity 10%
        ...
        const plotMigrations = async () => {
            ...
            for (const ancestor of ancestors) {
                ...
                if (birthCoords && deathCoords) {
                    addMigrationArrow(birthCoords, deathCoords, getOpacity(ancestor.relation_level));
                }
            }
        };
