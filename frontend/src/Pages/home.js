...
const HomePageWithTree = () => {
    ...
    const checkTreeEmpty = async () => {
      if (!currentTree !== null && currentTree !== undefined) {
        const response = await fetch(
          "http://localhost:5000/check-if-tree-empty",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  const homePageStats = useCallback(async () => {
    try {
      ...
      const occupationResponse = await fetch(
        "http://localhost:5000/count-occupations",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ currentTree }),
      ...
      const removedResponse = await fetch(
        "http://localhost:5000/get-most-removed-ancestor",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId }),
      ...
      const repeatedResponse = await fetch(
        "http://localhost:5000/get-most-repeated-ancestor",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId }),
      ...
      try {
        const progressResponse = await fetch(
          "http://localhost:5000/get-progress",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ userId }),
  ...
  useEffect(() => {
    if (mostRemovedAncestorRelation.length > 0 > 0) {
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const checkUserHasTrees = async () => {
      try {
        const response = await fetch(
          "http://localhost:5000/check-if-no-trees",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
