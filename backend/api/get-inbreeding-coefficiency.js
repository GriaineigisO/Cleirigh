    ...
    function calculateInbreedingCoefficient(personId, path = []) {
      ...
      if (person.father_id && person.mother_id) {
        ...
        for (const {
          ...
          // Calculate the inbreeding coefficient contribution from this common ancestor
          const F_CA = calculateInbreedingCoefficient(ancestorId, [
            ...path,
            personId,
          ]);
          // Use the base inbreeding contribution without adding 1 + F_CA
          commonCoEff += Math.pow(0.5, n + 1) * F_CA; // Adjust for distance from ancestor
          ...
          for (const s1 of steps1) {
            for (const s2 of steps2) {
              common.push({
                ...
                fatherSteps: s1 - 1, // Adjust steps to exclude direct parent
                motherSteps: s2 - 1, // Adjust steps to exclude direct parent
              });
            }
          }
        }
